...
@export var elevator_max_lift_coeficient : float = 1.0
@export var elevator_min_lift_coeficient : float = -1.0
...
func update_horizontal_stabilizers():
	...
	# lift calculation
	var lift_coefficient = remap(AoA + (elevator * 3), -15, 15, -1.2, 1.2)
	var lift_force = lift_coefficient * local_velocity.length_squared() * elevator_force_multiplier
	apply_force(Vector3(0,1,0) * lift_force, basis.z * -5)
	#apply_torque(basis.orthonormalized().x.normalized() * -lift_torque)
...
func update_wings():
	...
	var drag_coefficient = Vector3(0,1,0).dot(wing_local_velocity.normalized())
	...
	apply_central_force(wing_lift_force * basis.y.normalized())
